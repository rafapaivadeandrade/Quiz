...
type UserContextData = {
  ...
  update: (rating_score: number, duration_action: string, id: string) => void;
  calculateResult:(props:any, markedAnswers:any, durationTemp: any) => Promise<{ rating_score: number; duration_action: string;}>;
  loading:boolean;
...
export function UserContextProvider({ children }: UserContextProviderProps) {
  ...
  async function register(
    ...
    try {
        ...
        if (typeof window !== "undefined") {
          window.localStorage.setItem("userId", userId.data.response._id)
        }
        toast.success('Usuário cadastrado!');
        return userId.data.response._id;
    }
    catch (err:any) {
  ...
  async function update(
    rating_score: number, duration_action: string, id: string
  ) {
    try {
  ...
  async function calculateResult(props: any, markedAnswers: any[], durationTemp: any) {
    let time = moment.utc().valueOf()
    const duration = timeDifference(durationTemp, time)
    let correct = 0;
    props.forEach((question: { correctOptionIndex: any; points: number }, index: number) => {
