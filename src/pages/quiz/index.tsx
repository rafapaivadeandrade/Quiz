...
import { ContainerFormModal, FormBackgroundQuiz, ThankYouDiv } from '../../components/FormBackground';
...
export default function Quiz(props: any) {
  ...
  const [markedAnswers, setMarkedAnswers] = useState(new Array(props.shuffled2.length));
...
`;
  ...
  useEffect(() => {
    const saveFinalData = async () => {
      if (isQuestionEnd) {
        const data = await calculateResult(props.shuffled2, markedAnswers, durationTemp)
        setPoints(data.rating_score);
        if (typeof window !== "undefined") {
          const userId = window.localStorage.getItem("userId");
          const userIdParsed = userId as string;
          update((await data).rating_score, (await data).duration_action, userIdParsed)
              ...
              <ContainerFormModal result={true}>
                <ThankYouDiv>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <span>{points}</span>
                    <span>PONTOS</span>
                    <span>Obrigado pela participação e {'\n'}por ampliar o seu conhecimento {'\n'}nas doenças preveníveis pelas {'\n'}vacinas Pfizer</span>
                  </div>
                  {loading ? (
                    ...
                    <div className='ranking-div' onClick={() => {
...
export const getStaticProps: GetStaticProps = async (
  ...
  const shuffled2 = Object.keys(QuestionList as { "covid": [], "pneumonia": [], "Meningo": [], "Institucional": [] })
    .map(
      (questionType) => QuestionList[questionType as "covid" | "pneumonia" | "Meningo" | "Institucional"]
        .map(
          (question) => ({ question, sort: Math.random() })
        )
